...
如果上游只返回 `780` 而不是 `292/332`，打开“兼容新版 780 状态（实验）”。成功样本中业务响应会同时返回新的 `780` 和 `__cf_bm`，插件会滚动覆盖当前票据。跨出口带 Cookie 已有连续成功样本，但仍应对自己的账号做实际模型请求验收。
...
`780` 兼容模式遵循同样的复验规则，并保存复验响应中的滚动票。它与固定 `292` 不是互斥格式：在同一个 `session_id` 下，上游返回哪种合法长度，插件就保存哪种。
...
4. 检查诊断中的生成出口、业务出口、STATE 类别、响应模型和耗时。
