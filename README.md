...
| **插件版（新增）** | 已在使用官方 Sub2API 0.2.7，希望保留原版宿主 | `v4.0.0` 支持秒级续期和按账号优先复用上一轮可用生成器出口；正式安装包以最新 Release 为准，上传 `.s2plugin` 后在插件配置页逐账号设置 |
...
开启“优先复用账号上一轮可用出口 IP”后，插件会按账号记住上一次通过复验的生成器出口；下次打票先复用该出口，失败后自动回退到代理生成器。复用前仍会重新检查地区、出口一致性和票据复验。
